  ...
  useEffect(() => {
    ...
    const setupSocket = async () => {
      ...
      socketInstance.on('connect', () => {
        // console.log('[WS/Donate] ✅ Connected to server:', socketInstance.id);
        ...
        socketInstance.emit(SOCKET_IO_EVENT.join, joinPayload, (ack: unknown) => {
          // console.log('[WS/Donate] Joined:', ack);
        });
      });

      // ❌ Mất kết nối
      socketInstance.on('disconnect', (reason) => {
        // console.log('[WS/Donate] ❌ Disconnected / Server problem:', reason);
      });

      // ⚠️ Lỗi kết nối
      socketInstance.on('connect_error', (err: Error) => {
        console.error('[WS/Donate] ⚠️ Connection error:', err.message);
      });

      // 💰 Nhận donate realtime
      socketInstance.on(SOCKET_IO_EVENT.public_new_donate_by_room, (ev: WSPayload<RSBDonation>) => {
        // console.log(`[WS/Donate]💰 New received:`, ev);
